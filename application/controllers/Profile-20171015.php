...
class Profile extends MY_Controller {
    ...
    public function myphoto($user_id = null ) {
        if($this->is_login || $this->session->userdata('user_sr_info')){
            ...
            if($this->input->get("catalog") && $this->input->get("catalog") != ""){
            ...
            redirect(base_url("?action=signin"));
        }
    ...
    public function delete_photo($photo_id) {
        ...
        if ($this->Common_model->delete('photos', array(
            ...
            if (isset($record['path_file']) && $record['path_file'] != null && file_exists( base_url( $record['path_file'] ) ) ) {
                unlink('.' . $record['path_file']);
            }

            if (isset($record['thumb']) && $record['thumb'] != null && file_exists(base_url($record['path_file']))) {
                unlink('.' . $record['thumb']);
            }
            ...
            if (isset($_FILES['fileupload']) && is_uploaded_file($_FILES['fileupload']['tmp_name'])) {
                ...
                if ($image) {
                    ...
                    if ($data1["status"] == "success") {
                        ...
                        // --------------------------------------------------------------
	                    // ROOT PHOTO 
	                    //exec('rsync -azv ' . $path . ' /s3source/dezignwall/uploads/');
	                    //exec('rsync -azv ' . $thumb . ' /s3source/dezignwall/uploads/');
	                    exec('aws s3 sync —acl public-read ' . $path . ' s3://dwsource' . $output_url . ''); 
        ...
        if ($this->input->is_ajax_request() && isset($user_id) && $user_id != null && is_numeric($user_id)) {
            ...
            if($photo_id != null){
                ...
                if (file_exists($file)) {
                    ...
                    // --------------------------------------------------------------
                	// Upload to media.dezignwall.com
                	// rsync -azv /dw_source/dezignwall/uploads/<<FilenameHERE>> /s3source/dezignwall/uploads/
                	// exec('rsync -azv ' . $file . ' /s3source/dezignwall/uploads/');
                	// exec('rsync -azv ' . $file . ' /s3source/dezignwall/uploads/');
                	exec('aws s3 sync —acl public-read ' . $path . ' s3://dwsource' . $pathsync . ''); 
                        ...
                        if ($check) {
                            ...
                            if (isset($crop_data) && count($crop_data) > 0) {
                                ...
                                if (isset($crop_data['rotate']) && intval($crop_data['rotate']) > 0) {
                                    ...
                                    // --------------------------------------------------------------
				                    // CROP IMAGE
				                    exec('rsync -azv ' . $record_photo['path_file'] . ' /s3source/dezignwall/uploads/');
                                }
                                ...
                                // --------------------------------------------------------------
			                    // THUMBNAIL
			                    exec('rsync -azv ' . $record_photo['thumb'] . ' /s3source/dezignwall/uploads/');
                            }
                            //tag image
    ...
    public function batch_upload_file(){
        ...
        if($this->input->is_ajax_request() && $this->is_login && $this->input->post()){
            $manufacturers = $this->input->post("manufacturers-id");
                ...
                foreach ($data as $key => $value) {
                    ...
                    foreach ($value as $key_1 => $value_1) {
                        if($value_1 != null && array_diff_key($value_1, $data_check_key) == null){
                            $token = uniqid ()."-";
                            $data_logo = @$value_1['image URL'];
                            $path_info = pathinfo($data_logo);
                            $basename = $path_info['basename'];
                            $photo = null;
                            $thumb = null;
                            if($data_logo != null){
                                $img = file_get_contents($data_logo);
                                if($img){
                                    $path = FCPATH . 'uploads/member/';
                                    if (!is_dir($path)) {
                                        mkdir($path, 0755, TRUE);
                                    ...
                                    if(file_put_contents($file, $img)){
                                        ...
                                        $config['source_image'] = $file;
                                        $config['new_image'] = $path . "thumbs_".$token. $basename;
                                        $config['maintain_ratio'] = FALSE;
                                        $config['width'] = 400;
                                        $size_ratio = $this->ratio_image($w_current, $h_current, $config['width']);
                                        $config['height'] = $size_ratio['height'];
                                        ...
                                        $this->image_lib->initialize($config);
                                        $this->image_lib->resize();
                                        $photo = '/uploads/member/' .  $this->user_id . '/' . $token.$basename;
                                        $thumb = '/uploads/member/' .  $this->user_id . '/thumbs_'. $token.$basename;
                                        
                                        // --------------------------------------------------------------
					                    // ROOT PHOTO 
					                    exec('rsync -azv ' . $photo . ' /s3source/dezignwall/uploads/');
					                    exec('rsync -azv ' . $thumb . ' /s3source/dezignwall/uploads/');
                                    }
                                }     
                            }
