		...
		if ($body) {
			...
			foreach ($yahoo_array as $key => $values) {
				foreach ($values->contact as $keys => $values_sub) {
					if (isset($values_sub->fields[1])) {
						$email = @$values_sub->fields[1]->value;
						$nick = $email;
						if (!empty($email) && is_string($email) && strpos($email,'http://') === FALSE) {
							if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
								$email =  $email . '@yahoo.com';	
							}
							$fullname = trim(@$values_sub->fields[0]->value->givenName . ' ' . @$values_sub->fields[0]->value->familyName);
							$fullname = empty($fullname) ? $email : $fullname;
							$retarr[] = array(
									'name' => $fullname,
									'email' => $email,
									'image' => 'http://img.msg.yahoo.com/v1/displayImage/yahoo/' . $nick
								);
						}
					}
				}
