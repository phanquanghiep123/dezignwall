				    ...
				    <?php 						
						foreach ($like_record as $key_1 => $value) {
							if($key_1 < 6){
								...
								echo '
								<div class="media">
									<div class="media-left"> <a href="'.base_url("photos/".$value["reference_id"]."").'/'.gen_slug($value["name"]).'"><img src="'.$avatar.'" class="'.$value["typeSuccess"].' img-circle notification-avatar"></a> </div>
									<div class="media-body">
										<h5 class="media-heading remove-margin">'.implode(", ", $tow_member).'</h5>
										<h5 class="remove-margin">'.$count_like.'people like <a href="'.base_url("photos/".$value["reference_id"]."").'/'.gen_slug($value["name"]).'">your photo</a></h5> <span class="remove-margin color-blue">'.$created_at.'</span> 
									</div>
								</div>';
							}else{
								echo '<div class="row">
