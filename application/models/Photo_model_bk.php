...
class Photo_model extends CI_Model {
    ...
    function get_my_photo($member_id,$keyword = null,$cat_id = null,$offet = 0, $limit = 30,$in_id = null) {
        $sql_limit = "";
        if ($limit != 0) {
        ...
        FROM (SELECT * FROM photos AS pt WHERE `pt`.`type` IN(2, 3) AND `pt`.`member_id` = {$member_id} {$sql_keyword} {$sql_limit}) as `p`
    ...
    function total_photo($member_id,$keyword = null,$cat_id = null){
        $this->db->select('p.photo_id');
        $this->db->from($this->_table_photos . ' as p');
        if($cat_id != null){
            $this->db->join("photo_category as ptc", "ptc.photo_id = p.photo_id AND ptc.category_id = {$cat_id}");
        }
        $this->db->where(array(
    ...
    public function get_photo_slider_image_category($type = 'projects', $order_by, $limit) {
        ...
        $this->db->where("pt.status_photo", 1);
    ...
    public function search_index($member_id = 0, $count_record,$type ="",$slug="",$cat_id="") {
        $where = "";
        $join_categories = "";
        if ($type !== "") {
        ...
        if ($cat_id != "") {
            $where.= " AND (`ct`.`id` IN  (" . $cat_id . ") )";
            $join_categories = "    LEFT JOIN `photo_category` AS `ptct` ON `ptct`.`photo_id` = `pt`.`photo_id` 
                                    LEFT JOIN `categories` AS `ct` ON `ct`.`id` = `ptct`.`category_id` ";
        }
        $sql = "SELECT `ptl`.`id` AS member_total, tk.qty_like AS num_like, tk.qty_comment AS num_comment, 
				...
				`cp`.`company_name`,`cp`.`business_description` 
				FROM (SELECT pt.* FROM `photos` AS pt {$join_categories} WHERE (`pt`.`type` = 2) AND (pt.status_photo = 1) {$where} AND `pt`.`name` is not null ORDER BY  `pt`.`priority_display` ASC,`pt`.`created_at` DESC  LIMIT 0,{$count_record}) AS `pt2` 
				...
				LEFT JOIN `company` AS `cp` ON `cp`.`member_id`=`mb`.`id` GROUP BY `pt2`.`photo_id`  ORDER BY `pt2`.`priority_display` ASC,`pt2`.`created_at` DESC";
       
        $query = $this->db->query($sql);
        return $query->result_array();
    }
    public function total_page($location = "", $member_id = 0, $keyword = "", $categories = "", $type = "", $id_photo_show = "", $slug = "", $cat_id = "") {
        $text_category = "";
        $text_select = "";
        $text_id_photo_show = "";
        $where = " WHERE (`pt`.`type`= 2) AND (pt.status_photo = 1) AND (`pt`.`name` is not null) ";
        $text_in_location = "";
        $join_table_keyword = "";
        $keyword = trim($keyword);
        if ($keyword != "") {
            $join_table_keyword = "
                JOIN (SELECT tblefull.* FROM (SELECT pt.* 
                    FROM photos AS pt JOIN (SELECT table_2.* FROM ( 
                            (SELECT * FROM keywords AS kw WHERE `kw`.`title` LIKE '%{$keyword}%') AS table_1 
                            JOIN photo_keyword AS table_2 ON table_2.keyword_id = table_1.keyword_id 
                        )
                    ) AS ptkw ON ptkw.photo_id = pt.photo_id
                UNION

                SELECT pt3.* FROM photos AS pt3 WHERE pt3.`name` LIKE '%{$keyword}%' 
                
                UNION

                SELECT pt4.* FROM company AS cpm JOIN `photos` AS pt4 ON pt4.member_id = cpm.member_id AND cpm.`company_name` LIKE '%{$keyword}%' 

                ) AS tblefull) AS tbllike
                ON pt.photo_id = tbllike.photo_id
            ";
        }
        $join_table_category = "";
        ...
        if ((isset($categories) && count($categories) > 0 && $categories != "") || ($location != "" && is_array($location) && count($location) > 0)) {
            foreach ($categories as $key => $value) {
                if (trim($value) != "") {
                    $where .= " AND `pt`.`photo_id` IN(SELECT `ptct3`.`photo_id` FROM `photo_category` AS `ptct3` WHERE `ptct3`.`category_id` IN ('" . $value . "'))";
                }
            }
            if (is_array($location) && count($location) > 0) {
                $where.= " AND (`ptct`.`location` IN (0," . implode(",", $location) . ") )";
                $text_in_location = " AND `ptct`.`location` IN (0," . implode(",", $location) . ")";
            }
            $join_table_category = " JOIN `photo_category` AS `ptct` ON `ptct`.photo_id = `pt`.`photo_id` " . $text_in_location . " 
                                     JOIN `categories` AS `ct` ON `ct`.`id` = `ptct`.`category_id` ";

            $select_table_category = " `ptct`.`category_id`,`ptct`.`first_child_category`, ";
        }
        if ($type != "") {
            $where.= " AND (`pt`.`image_category` = '{$type}' OR `pt`.`image_category` = 'Projects,Products' OR `pt`.`image_category` = '')";
        }
        if (count($id_photo_show) > 0 && $id_photo_show != "" && is_array($id_photo_show)) {
            $where.= " AND `pt`.`photo_id` NOT IN (" . implode(',', $id_photo_show) . ")";
        }
        if ($slug != "") {
            ...
            if ($cat_id != "") {
                $where.= " OR (`ct`.`id` IN  (" . $cat_id . ") )";
            }
            $where.=")";
        }
        $sql = "SELECT `pt2`.`photo_id`
                FROM (SELECT pt.* FROM `photos` AS pt {$join_table_category} {$join_table_keyword} {$where}  GROUP BY `pt`.`photo_id` ORDER BY `pt`.`priority_display` ASC,`pt`.`created_at` DESC) AS `pt2` 
                INNER JOIN `members` AS `mb` ON `mb`.`id` = `pt2`.`member_id`";               
        $query = $this->db->query($sql);
        return $query->num_rows();
    }
    public function seach_photo($location = "", $member_id = 0, $keyword = "", $categories = "", $type = "", $offset = 0, $limit = 21, $id_photo_show = "", $slug = "", $cat_id = "") {
        $text_category = "";
        $text_select = "";
        $text_id_photo_show = "";
        $where = " WHERE (`pt`.`type`= 2) AND (pt.status_photo = 1) AND (`pt`.`name` is not null) ";
        ...
        $sql = "SELECT `ptl`.`id` AS member_total, tk.qty_like AS num_like, tk.qty_comment AS num_comment, 
				...
				FROM (SELECT pt.* FROM `photos` AS pt {$join_table_category} {$join_table_keyword} {$where}  GROUP BY `pt`.`photo_id` ORDER BY `pt`.`priority_display` ASC,`pt`.`created_at` DESC LIMIT $offset, $limit) AS `pt2` 
                LEFT JOIN common_tracking AS tk ON tk.reference_id = pt2.photo_id AND `tk`.`type_object`= 'photo' 
				LEFT JOIN common_like AS ptl ON ptl.reference_id = pt2.photo_id AND `ptl`.`member_id` = '{$member_id}' AND `ptl`.`type_object` = 'photo' AND `ptl`.`status` = 1 
				INNER JOIN `members` AS `mb` ON `mb`.`id` = `pt2`.`member_id` 				
				LEFT JOIN `company` AS `cp` ON `cp`.`member_id`=`mb`.`id` GROUP BY `pt2`.`photo_id` ORDER BY `pt2`.`priority_display` ASC,`pt2`.`created_at` DESC";
    ...
    function get_photos_like($keyword) {
        ...
        $this->db->where(array("pt.type" => 2,"pt.status_photo" => 1));
